...
The Greek philosopher and mathematician Plato compared human mind to an aviary in which each bird represented a memory~\cite{MichaelW.Eysenck2008}. Today, we have a much better intuition on the human biological storage, it's the hardware responsible for the storage and organization of data in modern computers. This analogy of computer data storage is way better than the one Plato used and it seems to go well with what actually happens when we retrieve an item from memory or forget something.

So far, we haven't mentioned learning, which is very closely related to memory as well as to the topic of our thesis. The study of learning and memory can be divided even further when we realize that we need some way to observe what the students know by experiments. All in all, this chapter is explained in three parts:

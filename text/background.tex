...
Throughout the last one hundred years, researchers have been trying to figure out the cognitive processes in our brain responsible for the ability to learn~\cite{RichardE.Mayer2010}. Understanding the process of learning can be very helpful in educational system, where we want to improve the student's representation in terms of their knowledge, behavior and response to a particular stimulus. The construction of a quantitative representation, called a \textit{student model,} is known as \textit{student modeling}~\cite{Sison1998}.
...
Richard E. Mayer formulated learning as a change in what the learner knows caused by the learner's experience~\cite{RichardE.Mayer2010}. More specifically from the brain's perspective, learning is the process of encoding information~\cite{Lewis}. For example the encoding of the location of Portugal can be seen as learning the shape of the country, its neighbor Spain and the surrounding ocean.
...
\begin{description}
  \item[Probability of recall] Probability of the student recalling the practiced item.
  \item[Latency of recall] Latency of the student retrieving the practiced item from their memory.
...
Memory is the biological storage that retains the information, for example the location of Portugal. The student's memory, however, decays with the time. This memory decay is called \textit{forgetting} and similarly as learning it respects the power law.
...
In the ACT-R model~\cite{Pavlik2003}, the memory strength $m$ of the student $s$ can be modeled by the Equiation~\ref{eq-actr}.
...
Performance is the speed and precision of recall (retrieval of an item from memory). The student's performance can be examined by a multiple-choice test, where the correctness of answers and response time is measured~\cite{Lewis}.
